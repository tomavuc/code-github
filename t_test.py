...
def welch_t_test_feature_selection(
    ...
    *,
    ...
    if exclude_ids is not None:
        ...
        if df.empty:
            raise ValueError("All rows were filtered out by `exclude_ids`.")

    df[label_col] = df[label_col].replace({"+/-": pos_label})
    df[label_col] = df[label_col].replace({pos_label: 1, neg_label: 0})
    if df[label_col].isna().any():
        raise ValueError("`label_col` contains unexpected values after mapping.")
    ...
    feature_cols = (
        ...
        .columns.difference([label_col] + list(exclude_cols)))
