...
def compute_binding_site_opening(cif_path, uniprot_id, threshold, chain_ids = ("A", "B")):
    parser = MMCIFParser()
    ...
    def _collect_atoms(chain, resnums):
        masses = []
        coords = []
    ...
    for chain_id in chain_ids:
        ...
        com_cys = _collect_atoms(chain, sites['CYS'])
        com_glu = _collect_atoms(chain, sites['GLU'])
    ...
    if abs(distances[0] - distances[1]) > threshold:
        print(f"Warning: binding-site openings for {uniprot_id} differ by {abs(distances[0] - distances[1]):.3f} Å")
    return avg_opening

def load_dimers(dimer_dir):
    structures = {}
    for fname in os.listdir(dimer_dir):
        if fname.endswith('.pdb') or fname.endswith('.cif'):
            key = fname.split('_dimer')[0]
            parts = key.split('_')
            # parts[1] is accession, parts[2] is version
            gb_id = f"{parts[1].upper()}.{parts[2]}"
...
if __name__ == "__main__":
    ...
    ecoli_structure_file = "e_coli_dimer.cif"
    ...
    for model_key, gb_id in dimers.items():
        ...
        if os.path.exists(cif_path) and os.path.exists(ecoli_structure_file):
            rmse_results[gb_id] = compute_rmse(cif_path, ecoli_structure_file)
            try:
